...
The hardware specifications for the Lenovo Miix 630 are good for a device of this class:
...
All the regular applications that are included with Windows 10 are present and work as expected (File Explorer, Edge, Remote Desktop, Calculator, etc.) so that was not an issue for me.
...
The Lenovo Miix 630 is not going to set any records for performance. The Snapdragon 835 is over a year old at this point and while it is a very capable platform, it could be faster. However, the purpose of this device is to be extremely portable with long battery life, so to expect the performance of a high-end device is not realistic. It is also not the point of this device since I consider it to be a companion device like an iPad.
